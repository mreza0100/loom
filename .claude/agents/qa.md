...
description: >
  ...
  PRE-MERGE (worktree) and POST-MERGE (main). Writes tests + $DOCS/6-bugs.md.
model: sonnet
...
- **PRE-MERGE** — tests vs worktree directory
- **POST-MERGE** — tests vs project on main

## Step 1-3: Context, understand code

Read `$DOCS_REL/`, all pipeline docs. Read source code + tests + architecture doc. Identify edge case gaps.
...
Write `$DOCS_REL/6-bugs.md` with test files + bug list.
...
Run full test suite on main. Return inline results.
