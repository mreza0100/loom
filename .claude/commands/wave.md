...
3. **Routing feasibility** — every task must be classifiable to a Loom subsystem (indexer, search, store, server, config). Too vague → fatal.
...
A task is JC if tagged `[CMD: /jc]` OR ALL of: touches ≤3 files, no code logic (config/constants/prompts/CLAUDE.md/env), no new files/schema/tests, no dependency on other tasks. Skip JC detection if `--no-jc` in `$ARGUMENTS`.
...
- Same-scope tasks → ONE pipeline
- Tasks with no conflicts/overlaps → prefer ONE pipeline (shares all overhead: planner, architect, QA, merge)
- Only separate when: real dependencies (B needs A's output), conflicting files, or tasks large enough to warrant own pipeline
- Merge single-pipeline sequential waves into ONE pipeline (one developer modifying a file once < two developers in sequence)
...
```
"Indexer improvements: fix parser edge cases, add caching, optimize embedder [Pipeline: indexer-v2] [Wave: {wave-name}]"
```
...
- **Group aggressively** — grouping IS the optimization. Never spawn 5 pipelines for 5 small tasks that could be one. Prefer ONE pipeline over separate pipelines when no conflicts.
...
- **Clean up** — verify no orphaned worktrees/branches after completion. Exception: BLOCKED-DEFERRED worktrees with matching `BLOCKED.md`.
