...
> Research protocol. The trigger is `RR` — short for "research and report".
...
| **codebase** | Read, Grep, Glob, Bash | Internal topics — "how is auth wired", "where do we use sqlite-vec", "find every place that touches embeddings", "audit our search pipeline" |
...
- "RR vector DBs" → probably means "which vector DB should we use for Loom given our stack and scale" — not a generic survey.
- "RR embedding models" → probably means "which embedding model best suits code search at Loom's scale" — not a Wikipedia summary.
...
**(Pre-identification shortcut.)** If during Step 1 the goal is already structured (e.g., "compare A, B, C" or "audit indexer, search, store"), you can pre-decide the sub-questions and skip the scout — go straight to fan-out at Step 4. Use the scout when the topic is open-ended; skip it when it isn't.
...
- **Self-contained.** No "as we discussed", no reference to prior turns, no assumption that the executor has read any file. If context matters (Loom's stack, scale, constraints, deadline), inline it in the prompt.
...
- Do **not** include secrets, internal URLs, or anything the user wouldn't paste into a fresh chat window.
...
- **Picking the wrong surface.** Codebase questions ("how does our indexer work") sent as internet research return generic blog posts; internet questions ("which embedding model should we use") sent as codebase research return "we don't have one." Pick deliberately. If unsure, ask.
